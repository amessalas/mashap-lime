...
def runtime_calculations(mashap_d, lime_d, datasets):
    ...
    with open('results/runtime.txt', 'w') as f:
        for ds in datasets:
            ...
            f.write(f"[{ds}] MASHAP was {t_l / t_m: .2f} times faster\n")

        t1 = np.mean(lime_time_mean)
        t2 = np.mean(mashap_time_mean)
        f.write(f"\n\nLIME overall mean runtime {int(t1)} seconds\n")
        f.write(f"MASHAP overall mean runtime {int(t2)} seconds\n")
        f.write(f"MASHAP was approximately {t1 / t2: .2f} times faster")
...
def t_test(excel):
    """
    Implement a student's t_test with a=10% on 6 aggregated metrics (see paper)
